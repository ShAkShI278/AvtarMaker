...
option_style = st.sidebar.selectbox('background style',('TRANSPARENT','CIRCLE'))
                                    
list_skin_color = ['TANNED','YELLOW','PALE','LIGHT','BROWN','DARK_BROWN','BLACK']
list_top_type = ['NO_HAIR','HAT','TURBAN','HIJAB',
...
list_hair_color = ['AUBURN','BLACK','BROWN','BLONDE','BLONDE_GOLDEN','BROWN_DARK',
                   'PASTEL_PINK','PLATINUM','RED','SILVER_GRAY']
...
list_facial_hair_type = ['DEFAULT','BEARD_MEDIUM','BEARD_LIGHT','BEARD_MAJESTIC','MOUSTACHE_FANCY']
...
list_eye_type = ['DEFAULT','CLOSE','CRY','DIZZY','EYE_ROLL','HAPPY','HEARTS','SIDE','SQUINT','SURPRISED']
list_eyebrow_type = ['DEFAULT','DEFAULT_NATURAL','ANGRY','ANGRY_NATURAL','FLAT_NATURAL','RAISED_EXCITED','RAISED_EXCITED_NATURAL','SAD_CONCERNED','SAD_CONCERNED_NATURAL','UP_DOWN','UP_DOWN_NATURAL','FROWN_NATURAL']
...
list_clothe_color = ['BLACK','BLUE_01','BLUE_02','BLUE_03','RED','WHITE','GRAY_01','PASTEL_BLUE','PASTEL_GREEN','PASTEL_ORANGE','PASTEL_RED','PASTEL_YELLOW','PINK']
...
#custom function for encoding and download the avatar  image
def imagedownload(filename):
    image_file = open(filename,'rb')
    b64 = base64.b64encode(image_file.read()).decode() #strings <_> bytes conversion
    href = f'<a href = "data:image/png;base64{b64}" download = {filename}>Download {filename} File</a>'
    return href

st.subheader('*Rendered Avatar*')
output_file = 'my_avatar.png'
rendered_avatar = avatar.render_png_file(output_file)
image = Image.open('my_avatar.png')
st.image(image)
st.markdown(imagedownload('my_avatar.png'),unsafe_allow_html=True)
